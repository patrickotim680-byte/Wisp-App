...
import { $, h, clear, toast, oops, modal, closeModal, promptBox, uuid, dur, debounce,
         icon, iconEl, initials, actionSheet } from './util.js';
...
export async function pushMessage(base) {
  ...
  try {
    ...
    // 42501 here is most often a stale access token (the tab sat backgrounded
    // long enough that autoRefreshToken missed its window) rather than a real
    // permission problem. One refresh-and-retry resolves that silently;
    // anything else still fails and falls through unchanged.
    if (e?.code === '42501') {
      try {
...
export function renderAttachRow() {
  ...
  S.pending.forEach(item => {
    const kill = h('button', {
      title: 'Remove', onclick: e => {
        e.stopPropagation();
        S.pending = S.pending.filter(x => x !== item);
        renderAttachRow();
      },
    });
    kill.append(iconEl('x', 12));
    ...
    if (item.kind === 'image' || item.kind === 'video') {
      thumb.title = 'Tap to make this view-once';
      thumb.onclick = () => {
        item.viewOnce = !item.viewOnce;
        thumb.style.outline = item.viewOnce ? '2px solid var(--accent)' : 'none';
        toast(item.viewOnce ? 'View once on — it disappears after one look' : 'View once off');
      };
    }
    row.append(thumb);
  });
  if (S.pending.length) row.append(h('small', { class: 'hint' }, 'Caption goes in the message box.'));
}
...
function attachMenu() {
  actionSheet('Attach', [
    { icon: 'image', label: 'Photos and videos', note: 'From this device', onclick: () => pickFiles('image/*,video/*') },
    { icon: 'camera', label: 'Camera', note: 'Take one now', onclick: () => pickFiles('image/*', 'environment') },
    { icon: 'doc', label: 'Document', note: 'Any file type', onclick: () => pickFiles('') },
    { icon: 'map-pin', label: 'Location', note: 'A pin, or live for a while', onclick: shareLocation },
    { icon: 'contact', label: 'Contact card', note: 'Share someone from your contacts', onclick: shareContact },
    { icon: 'poll', label: 'Poll', note: 'Ask the chat to pick', onclick: createPoll },
    { icon: 'sticker', label: 'Sticker', onclick: stickerPicker },
  ]);
}
...
function stickerPicker() {
  modal(h('h3', { class: 'display' }, 'Stickers'),
    h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '6px' } },
      STICKERS.map(s => h('button', {
        class: 'btn ghost', style: { fontSize: '34px' },
        onclick: async () => { closeModal(); try { await pushMessage({ kind: 'sticker', body: s }); } catch (e) { oops(e); } },
      }, s))),
...
async function shareLocation() {
  ...
  const live = await new Promise(res => actionSheet('Share location', [
    { icon: 'map-pin', label: 'Send current pin', onclick: () => res(0) },
    { icon: 'clock', label: 'Live for 15 minutes', onclick: () => res(15) },
    { icon: 'clock', label: 'Live for 1 hour', onclick: () => res(60) },
  ]));
...
async function shareContact() {
  ...
  try {
    ...
    actionSheet('Pick a contact', rows.map(r => ({
      icon: 'user', label: r.display_name,
      onclick: () => pushMessage({ kind: 'contact', meta: { name: r.display_name, user_id: r.id } }),
    })));
  } catch (e) { oops(e); }
}
  ...
  modal(h('h3', { class: 'display' }, 'New poll'), h('label', {}, 'Question', qIn), box,
    h('button', {
      class: 'btn small ghost', onclick: () => {
        const i = h('input', { placeholder: 'Option ' + (opts.length + 1) });
        opts.push(i); box.append(i); i.focus();
      },
    }, '+ Option'),
...
function closeEmoji() {
  $('.input-shell')?.querySelector('.emoji-pop')?.remove();
  $('#btn-emoji')?.classList.remove('armed');
}
function emojiPop() {
  const shell = $('.input-shell');
  if (shell.querySelector('.emoji-pop')) return closeEmoji();
  const pop = h('div', { class: 'emoji-pop' }, EMOJI_SET.map(e => h('button', {
    type: 'button', onclick: () => { insertAtCursor(e); closeEmoji(); },
  }, e)));
  shell.append(pop);
  $('#btn-emoji')?.classList.add('armed');
}
...
export function mountComposer() {
  ...
  el.addEventListener('keydown', e => {
    ...
    if (e.key === 'Escape') { S.replyTo = null; $('#reply-chip').hidden = true; $('#mention-pop').hidden = true; closeEmoji(); }
  });
  $('#btn-send').onclick = send;
  $('#btn-attach').onclick = attachMenu;
  $('#btn-emoji').onclick = e => { e.stopPropagation(); emojiPop(); };
  $('#btn-schedule').onclick = scheduleDialog;

  // The panel used to stay open until the same button was pressed again, so
  // tapping anywhere else left it floating over the thread.
  document.addEventListener('pointerdown', e => {
    if (e.target.closest?.('.emoji-pop') || e.target.closest?.('#btn-emoji')) return;
    closeEmoji();
  });

  const mic = $('#btn-mic');
  mic.addEventListener('pointerdown', startRec);
  mic.addEventListener('pointerup', () => { if (rec) toast('Recording — use Send or Discard when you are done.'); });
